...
class ProofOfStake:
    ...
    def __init__(self):
        self.stakers = {}  # {account => value}
        self.setGenesisNodeStake()

    def setGenesisNodeStake(self):
        genesisPublicKey = open('keys/genesisPublicKey.pem', 'r').read()
        self.stakers[genesisPublicKey] = 1  # only stake in system on genesis. so we set to 1


    def update(self, publicKeyString, stake):
        if publicKeyString in self.stakers.keys():
            self.stakers[publicKeyString] += stake # increment stake (should probably have to add a check here. like incr/decr stake)
        else:
            self.stakers[publicKeyString] = stake  # add stake if not in already. not sure if this is good method

    # get stake of specific publickey
    ...
    def winnerLot(self, lots, seed):
        winnerLot = None
        leastOffset = None
        referenceHashIntValue = int(BlockchainUtils.hash_(seed).hexdigest(), 16)
        for lot in lots:
